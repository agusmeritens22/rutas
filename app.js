...
const LS_KEY='saved_routes_timewin_v12_local';
...
function onImportNamed(file) {
  ...
  reader.onload = () => {
    try {
      ...
      data.forEach(r => { if (r && r.id && r.name) byId[r.id] = r; });
      const merged = Object.values(byId);
      saveSavedRoutes(merged);
      refreshSavedSelect();
      setStatus(`Importado ${merged.length} rutas ✅`);
      alert('Importación completa ✅');
    } catch (e) { console.error(e); alert('No se pudo importar: JSON inválido'); }
  };
...
async function geocodeAddress(addr){
  ...
  if (cfg.provider !== 'opencage' || !cfg.key) throw new Error('Geocoder no configurado (OpenCage).');
...
function planWithTimeWindows(stops, start, avgSpeedKmh, startMinutes){
  const remaining = stops.slice(); const route = []; let cur = start; let timeMin = startMinutes;
  while(remaining.length){
    ...
    chosen.__eta = Math.round(bestETA); chosen.__wait = Math.round(bestWait); chosen.__dep  = Math.round(bestETA + dwellMin);
    route.push(chosen); timeMin = chosen.__dep; cur = chosen;
  }
  return route;
}
...
function buildDirLinks(order,startPoint,circular=false,chunkTotal=10){
  const pts = []; if (startPoint && Number.isFinite(startPoint.lat) && Number.isFinite(startPoint.lng)) pts.push(startPoint);
...
$("updateNamed").addEventListener('click', async () => {
  ...
  if(updated){ try { await cloudSaveNamed(updated.name, updated.id); } catch(e){ console.error(e); } }
});
...
$("deleteNamed").addEventListener('click', async () => {
  ...
  onDeleteNamedLocal(); try { await cloudDelete(id); } catch(e){ console.warn("No se pudo borrar en la nube (quizás no existe):", e); }
});
...
document.addEventListener('keydown', (e) => {
  ...
  if ((isMac ? e.metaKey : e.ctrlKey) && e.key === 'Enter') { e.preventDefault(); $("updateBtn").click(); }
});

// ===== Firebase helpers (usa window._firebase del index) =====
async function cloudSaveNamed(name, forcedId){
  const fb = window._firebase; if(!fb){ console.warn("Firebase no inicializado"); return; }
  const { db, auth, doc, setDoc } = fb; const user = auth.currentUser; if(!user) { alert("Esperando autenticación…"); return; }
...
async function cloudList(){
  ...
  const { db, auth, collection, getDocs } = fb; const user = auth.currentUser; if(!user) return [];
  const snap = await getDocs(collection(db, "users", user.uid, "routes"));
  return snap.docs.map(d => d.data());
}
async function cloudDelete(id){
  const fb = window._firebase; if(!fb) return;
  const { db, auth, doc, deleteDoc } = fb; const user = auth.currentUser; if(!user) return;
...
async function cloudSyncToLocal(){
  ...
  } catch(e){ console.error("cloudSyncToLocal:", e); }
}
  ...
  const t = setInterval(() => {
    if (window._firebase && window._firebase.auth && window._firebase.onAuthStateChanged) {
      ...
      window._firebase.onAuthStateChanged(window._firebase.auth, async (user) => {
        if (user && !user.isAnonymous) { await cloudSyncToLocal(); }
      });
    }
